...
class TestLogin:
    ...
    async def test_successful_login_returns_token(self):
        ...
        with patch(
            "services.auth_service.account_repository",
        ) as mock_repo:
            mock_repo.find_by_credentials = AsyncMock(return_value=account)
            token = await login(login="alice", password="pw")
    ...
    async def test_wrong_credentials_raises(self):
        with patch(
            ...
            mock_repo.find_by_credentials = AsyncMock(return_value=None)
            with pytest.raises(AuthenticationError, match="Invalid login or password"):
                await login(login="alice", password="wrong")
    ...
    async def test_blocked_account_raises(self):
        ...
        with patch(
            ...
            mock_repo.find_by_credentials = AsyncMock(return_value=blocked)
            with pytest.raises(AccountBlockedError, match="blocked"):
                await login(login="alice", password="pw")
    ...
    async def test_login_calls_repo_with_correct_args(self):
        ...
        mock_repo.find_by_credentials.assert_awaited_once_with(

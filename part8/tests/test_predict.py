...
def _patch_infra(monkeypatch):
    ...
    fake_account = Account(id=1, login="test", password="hashed", is_blocked=False)
    main.app.dependency_overrides[get_current_account] = lambda: fake_account
    monkeypatch.setattr(main.app, "dependency_overrides", {get_current_account: lambda: fake_account})
...
def test_predict_success_is_valid_true(monkeypatch):
    ...
    monkeypatch.setattr(main, "get_model", lambda *_args, **_kwargs: FakeModel(0.9))
...
def test_predict_success_is_valid_false(monkeypatch):
    ...
    _patch_infra(monkeypatch)
    monkeypatch.setattr(main, "get_model", lambda *_args, **_kwargs: FakeModel(0.1))

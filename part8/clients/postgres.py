...
def track_db_query(query_type: str):
    """контекст менеджер, который записывает время выполнения запроса БД в гистограмму"""
...
async def get_pg_connection() -> AsyncGenerator[asyncpg.Connection, None]:
    ...
    connection: asyncpg.Connection = await asyncpg.connect(
        ...
        database=os.environ.get("PG_DATABASE", "hw"),
        host=os.environ.get("PG_HOST", "127.0.0.1"),
        port=int(os.environ.get("PG_PORT", "5435")),
    )

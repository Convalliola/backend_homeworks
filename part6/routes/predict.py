...
from errors import ModelNotLoadedError, AdvertisementNotFoundError
...
async def async_predict(
    ...
    async with get_pg_connection() as conn:
        ...
        if ad is None:
            raise AdvertisementNotFoundError(f"Ad with id={item_id} not found")
    ...
    async with get_pg_connection() as conn:
        ...
        if ad is None:
            raise AdvertisementNotFoundError(
                f"Ad with id={item_id} not found or already closed"
            )
